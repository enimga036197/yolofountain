...
import beam
from beam.crypto import BeamCryptoError
...
def test_password_roundtrip():
    ...
    tx = beam.Sender(payload, block_size=200, password="correct horse")
    rx = _deliver(tx)
    assert rx.done
...
def test_wrong_password_fails_cleanly():
    tx = beam.Sender(b"top secret" * 100, block_size=64, password="right")
    rx = _deliver(tx)
    try:
        rx.result(password="wrong")
        assert False, "wrong password should raise"
    except BeamCryptoError:
        pass


def test_missing_password_raises():
    tx = beam.Sender(b"secret" * 100, block_size=64, password="pw")
    rx = _deliver(tx)
    try:
        rx.result()
...
def test_stream_bytes_do_not_contain_plaintext():
    ...
    payload = secret * 40
    tx = beam.Sender(payload, block_size=64, password="pw", compress=False)
...
def test_encryption_survives_loss():
    ...
    tx = beam.Sender(payload, block_size=128, password="pw")
